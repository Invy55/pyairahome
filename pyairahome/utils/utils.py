...
class Utils:
                ...
                if isinstance(converted, dict): # process dict
                    for key, value in converted.items():
                        if hasattr(original, key): # ensure the key exists in the original message
                            if isinstance(getattr(original, key), uuid_pb2.Uuid): # if it's a UUID field
    ...
    def convert_to_uuid_list(device_ids):
        ...
        elif isinstance(device_ids, uuid_pb2.Uuid): # convert to list if given as single uuid
            heat_pump_ids = [device_ids]
    ...
    @staticmethod
    def convert_uuid_from_v2(device_id: str) -> uuid_pb2.Uuid:
        """ Convert a v2 UUID to protobuf format """
        return uuid_pb2.Uuid(value=UUID(device_id).bytes)
    
    @staticmethod
    def datetime_to_localdatetime(dt):
        return LocalDateTime(year=dt.year, month=dt.month, day=dt.day, hour=dt.hour, minute=dt.minute, second=dt.second, nanos=dt.microsecond * 1000)